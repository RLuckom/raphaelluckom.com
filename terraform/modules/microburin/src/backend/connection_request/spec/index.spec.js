...
const CONNECTION_REQUEST = checkAuth.__get__('CONNECTION_REQUEST')
...
describe("check auth", () => {
  ...
  it("rejects a request if the timestamp is in the future", async () => {
    const future = new Date().getTime() + 10000
    const evt = await validSignedTokenAuthEvent({origin: safeOrigin, requestType: CONNECTION_REQUEST, timestamp: future}, privateKey)
    return checkAuth.handler(evt).then((res) => {
  ...
  it("rejects a request if there is no timestamp", async () => {
    const evt = await validSignedTokenAuthEvent({origin: safeOrigin, requestType: CONNECTION_REQUEST}, privateKey)
    return checkAuth.handler(evt).then((res) => {
  ...
  it("rejects a request if the timestamp is too old", async () => {
    ...
    const evt = await validSignedTokenAuthEvent({timestamp: past, origin: safeOrigin, requestType: CONNECTION_REQUEST}, privateKey)
    return checkAuth.handler(evt).then((res) => {
  ...
  it("rejects a request if it is not signed for us", async () => {
    const now = new Date().getTime()
    const evt = await validSignedTokenAuthEvent({recipient: "anyone", timestamp: now, origin: safeOrigin, requestType: CONNECTION_REQUEST}, privateKey)
    return checkAuth.handler(evt).then((res) => {
  ...
  it("rejects a request if the connection already exists", async () => {
    ...
    const evt = await validSignedTokenAuthEvent({origin: safeOrigin, recipient: domain, timestamp: now, requestType: CONNECTION_REQUEST}, otherKey)
    return checkAuth.handler(evt).then((res) => {
      validateSuccess(res, STATUS_MESSAGES.existingConnection, safeOrigin, RESULTS.NOOP)
    })
  })
  ...
  it("rejects a request if the key takes longer than 1s to get", async () => {
    const now = new Date().getTime()
    queryTime = 2000
    const evt = await validSignedTokenAuthEvent({origin: safeOrigin, recipient: domain, timestamp: now, requestType: CONNECTION_REQUEST}, privateKey)
    return checkAuth.handler(evt).then((res) => {
