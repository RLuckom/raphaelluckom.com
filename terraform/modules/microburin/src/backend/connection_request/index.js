...
async function handler(event) {
  ...
  if (requestType !== CONNECTION_REQUEST) {
  ...
  if (connections.length) {
    return successResponse(STATUS_MESSAGES.existingConnection, origin, RESULTS.NOOP)
  }
  const signedString = Buffer.from(JSON.stringify({timestamp, origin, requestType: CONNECTION_REQUEST, recipient}), 'utf8').toString('base64').replace(/=*$/g, "")
  let signingKey
  try {
